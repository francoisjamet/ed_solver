MODULE bath_class
  ...
  TYPE bath_type
    ...
    INTEGER :: Nc      = 0
    LOGICAL :: SUPER   = F ! TRUE IF SUPERCONDUCTING
...
!**************************************************************************
  ...
  logical function which_basis_to_use__(mat)
   logical :: mat(:,:)
      ...
      if( .not.which_basis_to_use__.and.para_state        )  which_basis_to_use__ = .false.
      if( .not.which_basis_to_use__.and.ANY(mat)          )  which_basis_to_use__ = .true.
      if( .not.which_basis_to_use__.and.supersc_state     )  which_basis_to_use__ = .true.
      if( .not.which_basis_to_use__.and.force_sz_basis    )  which_basis_to_use__ = .true.
      if(      which_basis_to_use__.and.force_nupdn_basis )  which_basis_to_use__ = .false.

      write(145+rank,*) ' ---- > USING SUPERCONDUCTIVITY ? : ', which_basis_to_use__
...
!**************************************************************************
  ...
  SUBROUTINE new_bath_from_scratch(BATH,Nb,Nc,IMASKE,IMASKP,IMASKV,IMASKPV)
    ...
    INTEGER, OPTIONAL, INTENT(IN)    :: IMASKE(Nb,Nb,2),IMASKP(Nb,Nb),IMASKV(Nb,Nc,2),IMASKPV(Nb,Nc,2)
    INTEGER                          :: spin
...
!**************************************************************************
  ...
  SUBROUTINE new_bath_from_old(BATHOUT,BATHIN) 
    ...
    INTEGER, ALLOCATABLE           :: IMASKE(:,:,:),IMASKV(:,:,:),IMASKP(:,:),IMASKPV(:,:,:)
    INTEGER                        :: spin
    ...
    CALL new_bath_from_scratch(BATHOUT,BATHIN%Nb,BATHIN%Nc,IMASKE=IMASKE,IMASKP=IMASKP,IMASKV=IMASKV,IMASKPV=IMASKPV)
...
!**************************************************************************
  ...
  SUBROUTINE write_raw_bath(BATH)
    ...
    WRITE(UNIT,*) BATH%Nb ! redundancy to allow preallocation in read_from_raw_file
    WRITE(UNIT,*) BATH%Nc ! redundancy to allow preallocation in read_from_raw_file
...
!**************************************************************************
  ...
  SUBROUTINE read_bath(BATH,FILEIN,Nc)
    ...
    INTEGER,          INTENT(IN)    :: Nc
    INTEGER, ALLOCATABLE            :: IMASKE(:,:,:),IMASKV(:,:,:),IMASKP(:,:),IMASKPV(:,:,:)
    INTEGER                         :: Nb,index_in,ib,jb,spin,mu,iind_,iind,fac_cplx
    INTEGER                         :: Nitermax,ff
    REAL(DBL)                       :: step,dmax
 ...
 !---------------------------------------------------------------------------! 
    ...
    if(min_all_bath_param==0)then
      ...
      do spin=1,2
       ...
       do ib=1,Nb
        do jb=ib,Nb
          if((.not.diag_bath.or.ib==jb).or.(bath_nearest_hop.and.abs(ib-jb)<=1))then
           IMASKE(ib,jb,spin)=k
           k=k+1
          endif
        enddo
    ...
    call write_array( IMASKE, ' MASK BATH PARAMETERS EPSILON' , unit=log_unit )
...
#endif
       if(start_para.and.iind_>BATH%Eb(1)%rc%MASK%nind) val=(-1.d0+2.d0*dran_tab(iind-BATH%Eb(1)%rc%MASK%nind))/2.d0
       DO spin=1,2
         CALL fill_masked_matrix(BATH%Eb(spin),iind,val)
       ENDDO
     ENDDO
    endif
    call write_array( BATH%Eb(1)%rc%mat, ' BATH PARAMETERS EPSILON UP spin' , unit=log_unit ,short=.true. )
    call write_array( BATH%Eb(2)%rc%mat, ' BATH PARAMETERS EPSILON DN spin' , unit=log_unit ,short=.true. )
 ...
 !---------------------------------------------------------------------------!
    ...
    if(min_all_bath_param==0.or.force_pairing_from_mask)then
     if(min_all_bath_param/=0.and.force_pairing_from_mask)then
       CALL open_safe(UNIT2,'./ED/pairing','UNKNOWN','READ',get_unit=.true.)
 ...
 !---------------------------------------------------------------------------!
    ...
    call write_array( BATH%Pb%rc%MASK%imat, ' RVB PARAMETERS ', unit=log_unit)
    ...
    if(min_all_bath_param==0)then
     ...
     do spin=1,2
      ...
      do ib=1,Nb
       do mu=1,Nc
        if(fmos)Then
         if( .not. ( ib>(mu-1)*Nb/Nc .and. ib<=mu*Nb/Nc )  ) cycle
        endif
        IMASKV(ib,mu,spin)=k
        k=k+1
       enddo
 ...
 !---------------------------------------------------------------------------!
    ...
    if(min_all_bath_param==0)then
     DO spin=1,2
      CALL skip_line(UNIT,1)
      READ(UNIT,*) ((IMASKPV(ib,mu,spin),ib=1,Nb),mu=1,Nc)
     ENDDO
    else
     if(.not.force_no_bcs_pairing)then
     k=1
    ...
    call write_array( IMASKPV, ' MASK BATH PARAMETERS PVbc ' , unit=log_unit )
    endif
    endif
    endif
 ...
 !---------------------------------------------------------------------------!
...
#endif
       if(start_para.and.iind_>BATH%Vbc(1)%rc%MASK%nind) val=(-1.d0+2.d0*dran_tab(iind-BATH%Vbc(1)%rc%MASK%nind))
       DO spin=1,2
        CALL fill_masked_matrix(BATH%Vbc(spin),iind,val)
       ENDDO
      ENDDO
    ...
    call write_array( BATH%Vbc(1)%rc%mat, ' BATH PARAMETERS Vbc UP spin' , unit=log_unit ,short=.true. )
    call write_array( BATH%Vbc(2)%rc%mat, ' BATH PARAMETERS Vbc DN spin' , unit=log_unit ,short=.true. )
 ...
 !---------------------------------------------------------------------------!
    ...
    if(BATH%PVbc(1)%rc%MASK%nind+BATH%PVbc(2)%rc%MASK%nind>0)then
     if(min_all_bath_param==0)then
      CALL skip_line(UNIT,1)
      DO iind_=1,BATH%PVbc(1)%rc%MASK%nind+BATH%PVbc(2)%rc%MASK%nind
       READ(UNIT,*) iind,val
       DO spin=1,2
    ...
    call write_array( BATH%PVbc(1)%rc%mat, ' BATH PARAMETERS PVbc UP spin' , unit=log_unit ,short=.true. )
    call write_array( BATH%PVbc(2)%rc%mat, ' BATH PARAMETERS PVbc DN spin' , unit=log_unit ,short=.true. )
    endif
    endif
    endif
...
!**************************************************************************
  ...
  FUNCTION nbathparam(BATH)
    ...
    DO spin=1,2
      nbathparam = nbathparam + BATH%Eb(spin)%rc%MASK%nind + BATH%Vbc(spin)%rc%MASK%nind
    ENDDO
    do spin=1,2
...
!**************************************************************************
  ...
  FUNCTION nbathparam_(BATH)
    ...
    DO spin=1,2
      nbathparam_ = nbathparam_ + BATH%Eb(spin)%rc%MASK%nind + BATH%Vbc(spin)%rc%MASK%nind
    ENDDO
    DO spin=1,2
    ...
    DO spin=1,2
      nbathparam_ = nbathparam_ + BATH%Eb(spin)%rc%MASKoffdiag%nind + BATH%Vbc(spin)%rc%MASKoffdiag%nind
    ENDDO
    DO spin=1,2
...
!**************************************************************************
  ...
  SUBROUTINE write_bath(BATH,UNIT,FILEIN)
    ...
    IF(.NOT.ASSOCIATED(BATH%Eb)) STOP "ERROR IN write_bath: INPUT ISNT ASSOCIATED!"

    CALL dump_message(UNIT=UNIT,TEXT="#######################")
    CALL dump_message(UNIT=UNIT,TEXT="### BATH PARAMETERS ###")
    CALL dump_message(UNIT=UNIT,TEXT="#######################")

    IF(PRESENT(FILEIN)) CALL dump_message(UNIT=UNIT,TEXT="### READ FROM FILE "//TRIM(ADJUSTL(FILEIN)))

                      unit_ = log_unit
    IF(PRESENT(UNIT)) unit_ = UNIT

    WRITE(unit_,'(a,I0)') "# NUMBER OF SITES IN THE IMPURITY = ",BATH%Nc
    WRITE(unit_,'(a,I0)') "# TOTAL NUMBER OF BATH PARAMETERS = ",BATH%nparam
    ...
    WRITE(unit_,'(a,I0,a,E10.3)') "# Nparam = ",BATH%nparam," parameters,   TOLERANCE = ",BATH%dist_max
    WRITE(unit_,'(a,I0,a,E10.3)') "# Niter  = ",BATH%Niter_search_max," iterations with STEP = ",BATH%search_step
...
!**************************************************************************
  ...
  ! MAKE NAMBU ENERGY MATRIX 
    ...
    Nb = Eb(1)%rc%n1
    CALL new_masked_matrix(EbNambu,"EbNambu",Nb*2,Nb*2,IS_HERM=T)
    EbNambu%rc%mat = zero
    ...
    energy_global_shift2=-sum(diag(EbNambu%rc%mat(Nb+1:Nb*2,Nb+1:Nb*2)))
