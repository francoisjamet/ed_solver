MODULE DMFT_SOLVER_ED
  ...
  use solver
  use random,  only: init_rantab,modi,rand_init
  use minimization_wrapping, only: minloci
  use vertex
...
!  use linalg, only : root_equation
  ...
  PRIVATE
  PUBLIC :: stand_alone_ed,solver_ED_interface,initialize_ed_solver,finalize_ed_solver,write_begin_iter,dmft_standalone

  LOGICAL,      PARAMETER, PRIVATE           :: F=.FALSE.,T=.TRUE.

  CHARACTER(LEN=100),PRIVATE                 :: BATHfile
  CHARACTER(LEN=100),PRIVATE                 :: CORRELfile
  CHARACTER(LEN=100),PRIVATE                 :: EDfile

  LOGICAL            ,        PRIVATE        :: start_from_old_gs
  CHARACTER(LEN=100) ,        PRIVATE        :: OLDGSFILE

  TYPE(impurity_type),        PRIVATE,SAVE   :: impurity
  TYPE(bath_type),            PRIVATE,SAVE   :: bath
  TYPE(AIM_type),             PRIVATE,SAVE   :: AIM
  TYPE(eigensectorlist_type), PRIVATE,SAVE   :: GS
...
implicit none
 ...
 subroutine build1Dmesh(param,mesh,min,max)
...
!**************************************************************************
 ...
 subroutine solver_ED_interface(nbathparam_in,iter_,mmu,g_out,self_out,hybrid_in,Eimp,sigw,rdens,impurity_, &
                  & Himp,gw,frequ,flip_input_output,para_state_,retarded,restarted_,compute_all,tot_rep,    &
 ...
 integer           :: i,j,a1,a2,isector,iter_,nbathparam_in,ncpt_tot_back,ncpt_approx_tot_back
 integer,optional  :: nbathparam_
 real(8),optional  :: param_input_(nbathparam_in),param_output_(nbathparam_in),Jhund_
 ...
 real(8)           :: bandes(size(gw,1),size(frequ))
  ...
  !----------------------------------------------------------------------------------------------!
   ...
   if(present(freeze_poles_delta_iter_))     freeze_poles_delta_iter=freeze_poles_delta_iter_

                                             freeze_poles_delta=.false.
   if(present(freeze_poles_delta_))          freeze_poles_delta=freeze_poles_delta_

                                             only_compute_density =   .false.
   if(present(only_density))                 only_compute_density = only_density

                                             use_specific_set_parameters=.false.
   if(present(use_specific_set_parameters_)) use_specific_set_parameters=use_specific_set_parameters_
   ...
   if(.not.allocated(param_input))  allocate( param_input(bath%nparam+ncpt_para*ncpt_tot))
   if(present(param_input_)) then
     ...
     if(use_specific_set_parameters)then
      write(*,*) '---------------------------------------------------------'
      write(*,*) 'ED SOLVER, USE INPUT PARAMETER SPECIFIED : ', param_input
      write(*,*) '---------------------------------------------------------'
     endif
   endif
  ...
  !----------------------------------------------------------------------------------------------!
...
#else
     call update_impurity(impurity_%N,impurity,mmu,impurity_,Himp,Eimp=real(Eimp))
#endif
     ...
     if(.not.(iter_==1.and.(first_iter_use_edinput.or.start_from_old_gs)))then
     ...
     do isector=1,GS%nsector
       ...
       if(associated(GS%es(isector)%lowest%eigen)) write(log_unit,*) 'eigenvalues ...... ', GS%es(isector)%lowest%eigen(:)%val
     enddo
     ...
     CALL compute_correlations(AIM,GS,retarded,COMPUTE_ALL_CORREL=compute_all,only_dens=only_compute_density)
     ...
     if(present(imp_causality))then
       ...
       do j=1,size(self_out,1)
        do i=1,size(self_out,3)
          if(aimag(SNAMBU%fctn(j,j,i))>0.)    SNAMBU%fctn(j,j,i)=CMPLX(real(SNAMBU%fctn(j,j,i)),0.d0)
        enddo
        do i=1,size(sigww,3)
          if(aimag(SNAMBUret%fctn(j,j,i))>0.) SNAMBUret%fctn(j,j,i)=CMPLX(real(SNAMBUret%fctn(j,j,i)),0.d0)
        enddo
       enddo
       endif
     endif
  ...
  !----------------------------------------------------------------------------------------------!
    ...
    write(*,*) shape(gw_)
     ...
     do i=1,a1
      do j=1,a2
       if( .not.flip_input_output .or. .not. (i>a1/2.and.j>a2/2) )then
         call resampleit(real(GNAMBUret%correl(2,1)%freq%vec),   gw(i,j,:),frequ,  gw_(i,j,:),0.d0)
         call resampleit(real(GNAMBUret%correl(2,1)%freq%vec),sigww(i,j,:),frequ,sigw_(i,j,:),0.d0)
           gw(i,j,:) =           gw_(i,j,:)
         sigw(i,j,:) =         sigw_(i,j,:)
       else
         call mirror_array(real(GNAMBUret%correl(2,1)%freq%vec),   gw(i,j,:),frequ,  gw_(i,j,:))
         call mirror_array(real(GNAMBUret%correl(2,1)%freq%vec),sigww(i,j,:),frequ,sigw_(i,j,:))
...
144  continue
     ...
     call write_array( AIM%BATH%hybrid%fctn(:,:,i) , ' hybrid ', unit=30000   )
...
!**************************************************************************
 ...
 implicit none
 complex(8)  ::  vec(:),sig(:,:,:),green(size(T_cpt,1),size(T_cpt,2)),gg(size(vec),size(sig,1),size(sig,2))
 complex(8)  ::  g_out(:,:,:)
 integer     ::  i,j,k,l,nn
 logical     ::  imaginary
   ...
   do i=1,size(vec)
     ...
     green(1:size(sig,1),1:size(sig,2)) = green(1:size(sig,1),1:size(sig,2)) - sig(:,:,i)
     call invmat(nn,green)
     gg(i,1:size(sig,1),1:size(sig,2))=green(1:size(sig,1),1:size(sig,2))
   enddo
...
!**************************************************************************
 ...
 type(hamiltonian)       :: Himp
 type(web)               :: impurity_
 real(8),allocatable     :: tre(:),tim(:)
 real(8),allocatable     :: tref(:),timf(:)
 integer                 :: i,nn,ii,jj,kk,nmatsu_frequ,nmatsu_long,FLAGIMP,nstep,Nww,bath_param_ed,FLAG_AVERAGE_G,FLAG_ORDER_TYPE
 logical                 :: retarded,para_state_,supersc_state_,compute_all
 real(8)                 :: tot_rep,frequ_min,frequ_max,dU1,ttemp,JJhund
 ...
 complex(8),allocatable  :: spm_(:,:,:),corhop_(:,:,:),hybrid_in(:,:,:),Eimp(:,:),hybrid_in_long(:,:,:)
 complex(8),allocatable  :: self_out(:,:,:),sigw(:,:,:),gw(:,:,:),g_out(:,:,:)
 ...
 allocate(tre(cluster_problem_size),tim(cluster_problem_size))
 allocate(tref(cluster_problem_size**2),timf(cluster_problem_size**2))
 ...
 do ii=1,cluster_problem_size
    read(10001,*) (Eimp_(ii,jj),jj=1,cluster_problem_size)
 enddo
 Eimp(cluster_problem_size+1:2*cluster_problem_size,cluster_problem_size+1:2*cluster_problem_size)=Eimp_
 do ii=1,cluster_problem_size
 ...
 allocate(spm_(2*cluster_problem_size,2*cluster_problem_size,nmatsu_long),corhop_(2*cluster_problem_size,2*cluster_problem_size,nmatsu_long))
 allocate(g_out(2*cluster_problem_size,2*cluster_problem_size,nmatsu_long))
 allocate(tempmat(cluster_problem_size,cluster_problem_size))
 allocate(hybrid_in(2*cluster_problem_size,2*cluster_problem_size,nmatsu_frequ),hybrid_in_long(2*cluster_problem_size,2*cluster_problem_size,nmatsu_long))
 allocate(self_out(2*cluster_problem_size,2*cluster_problem_size,nmatsu_long))
 allocate(sigw(2*cluster_problem_size,2*cluster_problem_size,nreal_frequ))
 ...
 call build1Dmesh(frequr,nreal_frequ,frequ_min,frequ_max)
  ...
  !optional arguments....
   JJmatrix_loc=Jhund
   do ii=1,cluster_problem_size
    read(10001,*,end=61) (JJmatrix_loc(ii,jj),jj=1,cluster_problem_size)
   enddo
 ...
 do ii=1,cluster_problem_size
  do jj=1,cluster_problem_size
   if(abs(UUmatrix_loc(ii,jj))<1.d-6.and.abs(JJmatrix_loc(ii,jj))>1.d-6)then
    write(*,*) 'ERROR zero repulsion (so mask repulsion will be false)'
    write(*,*) 'and finite hunds coupling, i,j : ', i,j
    write(*,*) 'U,J : ', UUmatrix_loc(ii,jj),JJmatrix_loc(ii,jj)
    write(*,*) 'they share the same mask within the code, this should match, critical'
    stop
   endif
  enddo
 enddo
  ...
  do jj=1,cluster_problem_size
   if(ii/=jj.and.abs(UUmatrix_loc(ii,jj))>1.d-6)then
    if(.not.Jhund_Slater_type)then
       UUmatrix_loc(ii,jj) = UUmatrix_loc(ii,jj) - 2.5d0*JJmatrix_loc(ii,jj)
    else
       UUmatrix_loc(ii,jj) = UUmatrix_loc(ii,jj) - 2.0d0*JJmatrix_loc(ii,jj)
    endif
   endif
  enddo
 enddo
 ...
 if(.not.allocated(Himp%dU))          allocate(Himp%dU(1),Himp%eps(1),Himp%teta(1,1))
 if(.not.allocated(impurity_%nneigh)) allocate(impurity_%nneigh(1),impurity_%site(cluster_problem_size))
 if(.not.allocated(Himp%Vrep))        allocate(Himp%Vrep(1,1))

 impurity_%N=cluster_problem_size
 impurity_%nneigh=0
 if(.not.allocated(impurity_%cadran)) allocate(impurity_%cadran(impurity_%N,1))
 impurity_%cadran=0
 impurity_%site=1
 impurity_%teta=0
 impurity_%nneigh=1
 ...
 write(*,*) '        because NO_SYS_CALL was set'
  ...
  if(.not.check)then
  INQUIRE(file='delta_input1',EXIST=check)
  if(check)then
  open(unit=10001,file='delta_input1')
  j=0
  do
    read(10001,*,end=66) ttemp,(tre(k),tim(k),k=1,cluster_problem_size)
    write(*,*) ttemp,(tre(k),tim(k),k=1,cluster_problem_size)
    j=j+1
    frequi(j)=ttemp
    do k=1,cluster_problem_size; hybrid_in(k,k,j)=cmplx(tre(k),tim(k)); end do;
  enddo
  66 continue
 ...
 INQUIRE(file='delta_input2',EXIST=check)
  if(check)then
  open(unit=10001,file='delta_input2')
  j=0
  do
    read(10001,*,end=667) ttemp,(tre(k),tim(k),k=1,cluster_problem_size)
    write(*,*) ttemp,(tre(k),tim(k),k=1,cluster_problem_size)
    j=j+1
    frequi(j)=ttemp
    do k=1,cluster_problem_size; hybrid_in(cluster_problem_size+k,cluster_problem_size+k,j)=cmplx(tre(k),tim(k)); end do;
  enddo
  667 continue
  if(j/=nmatsu_frequ)then
   write(*,*) 'not the right number of matsubara frequencies in delta_input_full'
   stop
  endif
  close(10001)
  endif
...
if(skip_fit)then
  ...
  do
    ...
    read(10001,end=666) ttemp,tempmat
    j=j+1
    frequi(j)=ttemp
  ...
  frequi=(/( (2*j-1)*pi/bbeta,j=1,nmatsu_frequ )/)
  endif
...
112 continue
  ...
  if(use_input_delta)then
    write(*,*) 'WARNING WARNING WARNING using input delta to compute Self, instead of fitted delta'
    write(*,*) 'this will break causality'
  endif

 if(.not.cluster_full)then
  write(*,*) 'WARNING WARNING WARNING removing off diag elements in ED solver'
  do j=1,nmatsu_frequ
  ...
  if(rank==0)then
   do j=1,nmatsu_frequ
    if(maxval(abs(  hybrid_in(:,:,j) - transpose(hybrid_in(:,:,j)) )) >1.d-3)then
      write(*,*) 'WARNING hybridization not symmetric !!!'
       write(*,*) 'frequ : ', j
       write(*,*)
       do k=1,cluster_problem_size
          write(*,'(200f8.3)') (abs(  hybrid_in(k,l,j) - hybrid_in(l,k,j) ),l=1,cluster_problem_size)
       enddo
       write(*,*) '---------real--------------'
       do k=1,cluster_problem_size
          write(*,'(200f8.3)') (real(  hybrid_in(k,l,j) ),l=1,cluster_problem_size)
       enddo
       write(*,*) '---------imag--------------'
       do k=1,cluster_problem_size
          write(*,'(200f8.3)') (aimag(  hybrid_in(k,l,j) ),l=1,cluster_problem_size)
       enddo
      !stop
    endif
 enddo
...
!call plotarray(frequi,aimag(hybrid_in(1,4,:)),'offdiag14')
 ...
 !#########################
   ...
   call initialize_ed_solver(mmu,bbeta,impurity_,Himp,rdelta_width,nmatsu_long,                                           &
                        & FLAGIMP,nstep,nreal_frequ,frequ_min,frequ_max,para_state_,FLAG_ORDER_TYPE==3,bath_param_ed,     &
                        & rdelta_frequ_eta1_=rdelta_frequ_eta1_,rdelta_frequ_T_=rdelta_frequ_T_,                          &
                        & rdelta_frequ_w0_=rdelta_frequ_w0_,init=init_every_time.or.first_time,                           &
                        & average_G_=FLAG_AVERAGE_G,UUmatrix_in_size=cluster_problem_size,UUmatrix_in=UUmatrix_loc,       &
                        & fit_green_func_and_not_hybrid_=fit_green,donot_compute_holepart_=nohole)
 ...
 if(check)then
  ...
  if(.false.)then
    78 continue
    write(*,*) 'the size of the ED solver bath has changed, erasing fitting parameters file'
    close(1212)
#ifndef NO_SYS_CALL
    if(rank==0.or.no_mpi)then
 ...
 if(bath_param_ed==0)then
  write(*,*) 'error initialization problem, 0 bath param : ', bath_param_ed
 endif
 if(rank==0)then
  write(*,*) '############################################'
 ...
 if(rank==0)then
 call write_array(real(self_out(:,:,nmatsu_long)),'self at matsu long RE',short=.true.,unit=6)
 call write_array(real(g_out(:,:,nmatsu_long)),'g at matsu long RE',short=.true.,unit=6)
 call write_array(abs(real(self_out(:,:,nmatsu_frequ-1)-self_out(:,:,nmatsu_long))), &
              & ' correction of Eimp nmatsu_long-nmatsu_frequ RE',short=.true.,unit=6)
 call write_array(aimag(self_out(:,:,nmatsu_long)),'self at matsu long IM',short=.true.,unit=6)
 call write_array(aimag(g_out(:,:,nmatsu_long)),'g at matsu long IM',short=.true.,unit=6)
 call write_array(abs(aimag(self_out(:,:,nmatsu_frequ-1)-self_out(:,:,nmatsu_long))), &
              & ' correction of Eimp nmatsu_long-nmatsu_frequ IM',short=.true.,unit=6)
 endif
 ...
 if(no_cdw)then
   ...
   do j=1,nmatsu_frequ
    do ii=1,nn
     do jj=1,nn
      if(jj>nn/4.and.ii>nn/4.and.ii<=nn/2.and.jj<=nn/2)then
 ...
 open(unit=10001,file='_sigma_output_full_real_1',form='unformatted')
  k=cluster_problem_size
  do j=1,nreal_frequ
    write(10001)   sigw(1:k,1:k,j)
  enddo
 close(10001)
 open(unit=10001,file='_sigma_output_full_real_2',form='unformatted')
  k=cluster_problem_size
  do j=1,nreal_frequ
...
!============= with more frequencies ===========!
 open(unit=10001,file='_sigma_output_full_long_1',form='unformatted')
  k=cluster_problem_size
  do j=1,nmatsu_long
    write(10001)   self_out(1:k,1:k,j)
  enddo
 close(10001)
 open(unit=10001,file='_sigma_output_full_long_2',form='unformatted')
  k=cluster_problem_size
  do j=1,nmatsu_long
    write(10001)   self_out(k+1:2*k,k+1:2*k,j)
  enddo
...
!============= with more frequencies ===========!
 ...
 do j=1,nreal_frequ
  ...
  enddo
  close(10001)
  if(.not. FLAG_DUMP_INFO_FOR_GAMMA_VERTEX) then
 ...
 do j=1,nmatsu_frequ
  ...
  do k=1,2
  open(unit=10001,file='green_output_matsu_full'//TRIM(ADJUSTL(toString(k))),form='unformatted')
    do j=1,nmatsu_frequ
...
!============= with more frequencies ===========!
 do k=1,2
  open(unit=10001,file='green_output_matsu_full_long_'//TRIM(ADJUSTL(toString(k))),form='unformatted')
    do j=1,nmatsu_long
       i=cluster_problem_size
       if(k==1)then
         write(10001) g_out(1:i,1:i,j)
...
!============= with more frequencies ===========!
  ...
  deallocate(    UUmatrix,JJmatrix,rdens                    &
              & ,frequr,frequi,tempmat                      &
              & ,spm_,corhop_,hybrid_in,Eimp,hybrid_in_long &
              & ,self_out,sigw,gw,g_out                     &
              & ,temp,Eimp_,bathparams,bathparams_output)
...
!**************************************************************************
  ...
  SUBROUTINE initialize_ed_solver(mmu,bbeta,impurity_,Himp,rdelta_width,nmatsu_frequ,                     &
                                & FLAGIMP,nstep,Nww,wwmin,wmax,para_state_,supersc_state_,bath_param_ed,  &
                                & rdelta_frequ_eta1_,rdelta_frequ_T_,rdelta_frequ_w0_,init,average_G_,UUmatrix_in_size, &
                                & UUmatrix_in,fit_green_func_and_not_hybrid_,donot_compute_holepart_)
  implicit none
  ...
  logical,optional   ::  init,fit_green_func_and_not_hybrid_,donot_compute_holepart_
  ...
  integer :: ii
    ...
    if(flag_slater_int)Then
     ...
     if(chan/=impurity_%N)then
       write(log_unit,*) 'ERROR, slater file contains [x] orbital sets : ', chan
       write(log_unit,*) '       and  impurity model contains [x] orb  : ', impurity_%N
       stop
     endif
     if(chan>7)then
     ...
     if(allocated(Slater_Coulomb_c)) deallocate(Slater_Coulomb_r,Slater_Coulomb_c)
     allocate(Slater_Coulomb_c(chan,chan,chan,chan),Slater_Coulomb_r(chan,chan,chan,chan))
     read(11122) Slater_Coulomb_c
     close(11122)
     Slater_Coulomb_r=Slater_Coulomb_c
#ifndef _complex
     if(maxval(abs(Slater_Coulomb_c-Slater_Coulomb_r))>1.d-2)then
       write(*,*) 'Slater Coulomb interaction is complex, and running Lanczos in real mode'
       write(*,*) 'please recompile with _complex flag, to run the complex Lanczos'
       write(*,*) 'maxval Slater_Coulomb_c : ',maxval(abs(Slater_Coulomb_c))
     ...
     if(use_precomputed_slater_matrix)then
       ...
       if(g4/=chan) then
         write(*,*) 'OUPS, the file Uc.dat.ED.matrix was for [x] orbitals : ', g1
         stop
       endif
...
!    call generate_UC_dat_files(UUmatrix_in_size,Slater_Coulomb_c)
    ...
    if(present(UUmatrix_in))then
       ...
       write(*,*) ' size of correlation matrix is (those two should be the same) : ', UUmatrix_in_size
       if(flag_slater_int) UUmatrix=0.d0
       call write_array(UUmatrix,' repulsion matrix input, please check and comment out ', short=.true.,unit=6)
    ...
    if(present(fit_green_func_and_not_hybrid_)) fit_green_func_and_not_hybrid=fit_green_func_and_not_hybrid_
    ...
    if(present(init)) then
      if(init) CALL read_DMFT_parameters(nmatsu_frequ,FLAGIMP,nstep,impurity_%N)
    endif
    ...
    if(supersc_state.and..not.force_nupdn_basis) min_all_bath_param=-abs(min_all_bath_param)
    ...
    CALL init_correlations(impurity,bbeta,CORRELfile,rdelta_width,wmin,wmax,nmatsu_frequ,Nww)
    write(log_unit,*) '================================================'
    ...
    if(ncpt_approx/=0)then
      ...
      ncpt_tot        = AIM%Nb * ( ncpt_approx*(ncpt_approx+1)/2) + AIM%Nb*ncpt_chain_coup
    else
      ncpt_approx_tot = 0
      ncpt_tot        = 0
      ncpt_chain_coup = 0
    ...
    write(log_unit,*) ' NUMBER OF BATH PARAMETERS : ', bath_param_ed+ncpt_para*ncpt_tot
