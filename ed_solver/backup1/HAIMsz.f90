MODULE HAIMsz_class
  ...
  INTEGER                          , PRIVATE :: istatemin,istatemax,dchunk,dimen
...
!**************************************************************************
  ...
  !------------------!
    ...
    if(.not.ON_FLY) CALL tab_HAIM2(AIM2sz,sector_sz)   ! TABULATE QUADRATIC PART
  ...
  !---------------------------------------------------------------!
    ...
    DO istate=istatemin,istatemax ! span the local chunk of the sector
      ...
      state     = sector_sz%state(istate)
       ...
       ! ONSITE COULOMB
      DO site1=1,Nc
       bn1 = (/BTEST(state,IMPiorb(site1)-1),.NOT.BTEST(state,IMPiorb(site1+Nc)-1)/)
       n1  = COUNT(bn1)
       IF(n1==2.AND.U%MASK%mat(site1,site1)) QUART_INT_SZ(istateloc) = QUART_INT_SZ(istateloc) + U%mat(site1,site1)

       ! INTER-SITE COULOMB
       IF(offdiag_coulomb.AND.n1/=0)THEN
         DO site2=site1+1,Nc
         IF(U%MASK%mat(site1,site2))THEN
           bn = (/BTEST(state,IMPiorb(site2)-1),.NOT.BTEST(state,IMPiorb(site2+Nc)-1)/)
           n2 = COUNT(bn)
           QUART_INT_SZ(istateloc) = QUART_INT_SZ(istateloc) + U%mat(site1,site2) * n1 * n2
         ENDIF
         ENDDO
       ENDIF
       ...
       IF(abs(Jhund)>1.d-5.and.Jhund_Slater_type)THEN
         DO site2=site1+1,Nc
            bn2 = (/BTEST(state,IMPiorb(site2)-1),.NOT.BTEST(state,IMPiorb(site2+Nc)-1)/)
            if(bn1(1).and.bn2(1))then
              QUART_INT_SZ(istateloc) = QUART_INT_SZ(istateloc) - JJmatrix(site1,site2)
            endif
...
!**************************************************************************
  ...
  !-------------------------------------------------------!
    ...
    INTEGER                     :: istateloc,istate,jstate,noff,irank,istatemin0,istatemax0,imin_(MAXT),imax_(MAXT),TID,istatemin_,j
    INTEGER(8)                  :: nhoffdiag
    INTEGER                     :: Nc
    integer                     :: site1,site2,n1,n2,i1,i2,ket_in_,ket_out_,jj,fermion_sign_
   ...
   if(OPEN_MP) then
     ...
     where(imax_/=0) imax_=imax_+istatemin-1
                ...
                go_for_omp=.false.
    if(OPEN_MP) go_for_omp=minval(imax_)>0.and.minval(sector_sz%chunk)>0.and.minval(imin_)>0

   !$OMP PARALLEL IF(go_for_omp) PRIVATE(site1,site2,n1,n2,i1,i2,ket_in_,ket_out_,jj,fermion_sign_,bn1,bn2,hoffdiag,TID,istatemin_,istatemin0,istatemax0,istate,istateloc,noff,irank,jstate,nhoffdiag) SHARED(vec_out,vec_in,noffsz,rankoffsz,offdiagsz)
   ...
   DO istate=istatemin0,istatemax0 
      ...
      if(.not.USE_CC)then
        vec_out(istate) = vec_out(istate) + QUART_INT_SZ(istateloc) * vec_in(istate) 
      else
        vec_out(istate) =                   QUART_INT_SZ(istateloc) * vec_in(istate)
      endif
      vec_out(istate) = vec_out(istate) +     diagsz(istateloc) * vec_in(istate)
      ...
      DO irank=1,noff
        ...
        if(.not.USE_CC) &
&       vec_out(jstate) = vec_out(jstate) +       hoffdiag  * vec_in(istate)
       ...
       DO istate=istatemin0,istatemax0
         ...
         istateloc = istate - istatemin_ + 1
            ...
            DO site1=1,Nc
                ...
                bn1 = (/BTEST(sector_sz%state(istate),IMPiorb(site1)-1),BTEST(sector_sz%state(istate),IMPiorb(site1+Nc)-1)/)
                n1  =  COUNT(bn1)

                DO site2=site1+1,Nc

                  bn2= (/BTEST(sector_sz%state(istate),IMPiorb(site2)-1),BTEST(sector_sz%state(istate),IMPiorb(site2+Nc)-1)/)
                  n2 = COUNT(bn2)
                     ...
                     ! -2J Si^z Sj^z : up up --> -J/2 (ni-1) (nj-1)
                        vec_out(istate) = vec_out(istate) - 0.50d0 * JJmatrix(site1,site2) * dble( n1-1 )*dble( n2-1 )*vec_in(istate)
                     endif
                ...
                DO site2=1,Nc
                 if(site1/=site2)then
                   ...
                   !-------------------------------------------------------!
                        ...
                        ket_out_  = IBCLR(ket_out_,IMPiorb(site2+Nc)-1) !do
...
35 continue
   ...
   if(HILBERT_SPACE_SPLITED_AMONG_NODES) then
     ...
     if(USE_CC.and..not.emptychunks)then
      call mpibcast(vec_out,sector_sz%chunk(:),[(sum(sector_sz%chunk(1:j-1)),j=1,size2)]   )
     else
...
!**************************************************************************
  ...
  !-------------------------------------------------------!
   ...
   !$OMP PARALLEL IF(go_for_omp) PRIVATE(hoffdiag,TID,istatemin_,istatemin0,istatemax0,istate,istateloc,noff,irank,jstate,nhoffdiag) SHARED(vec_out,vec_in,noffsz,rankoffsz,offdiagsz)
...
!**************************************************************************
  ...
  !-------------------------------------------------------!
    ...
    REAL(DBL),    INTENT(IN)    :: vec_in(:)
    REAL(DBL)                   :: hoffdiag
    INTEGER                     :: istateloc,istate,jstate,irank,istatemin0,istatemax0,imin_(MAXT),imax_(MAXT),TID,istatemin_,j,iorb,jorb
    INTEGER                     :: IMPnorbs,BATHnorbs,i,ket_in_,ket_out_,norbs_,jj,n1,n2
   ...
   !$OMP PARALLEL IF(go_for_omp) PRIVATE(hoffdiag,TID,istatemin_,istatemin0,istatemax0,istate,istateloc,irank,jstate,iorb,jorb,i,jj,j,ket_in_,ket_out_,fermion_sign_,norbs_) SHARED(vec_out,vec_in,QUART_INT_SZ)
  ...
  !--------------------------------------------------------------------------------!
   ...
   DO istate=istatemin0,istatemax0
      ...
      DO iorb=1,IMPnorbs
        IF(BTEST(ket_in_,IMPiorb(iorb)-1))  vec_out(istate) = vec_out(istate) + AIM2sz%Ec%rc%mat(iorb,iorb)*vec_in(istate)
      ENDDO
      DO iorb=1,BATHnorbs
